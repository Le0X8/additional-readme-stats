...
for ($i = 0; $i < $limit; $i++) {
    $description .= "\t" . ($i + 1) . '. ' . htmlspecialchars($artists[$i]['name']) . "\n\t";

    $contents .= ($hide_rank ? '' : '<text y="' . (67.5 + $i * 40) . '"' . ($i > 8 ? ' x="-7.5"' : '') . ' class="rank">' . $i + 1 . '</text>') . '<image href="data:image/jpeg;base64,' . base64_encode(file_get_contents($artists[$i]['img'])) . '" y="' . (48.75 + $i * 40) . '" clip-path="url(#clip' . $i . ')" class="image"></image>
        <text y="' . (67.5 + $i * 40) . '" class="name">' . htmlspecialchars($artists[$i]['name']) . '</text>';
};
...
echo (str_replace([
    ...
    $COLORS['inner_border_radius'],
    htmlspecialchars($title),

    htmlspecialchars($description),
    $contents
], file_get_contents('src/imgs/spotify/artists.svg')));
