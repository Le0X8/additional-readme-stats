...
export async function GET({ url }) {
	try {
		...
		const recents = await getRecents(username, limit);

		for (let i = 0; i < limit; i++) {
			recents[i].img = await dataUrl(recents[i].img);
		}
		...
		const title = theme.custom_title ?? 'My recently played tracks';

		return text(
			`<svg width="350" height="${height}" viewBox="0 0 350 ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img">
    <title>${title}</title>
    <desc>${recents.map((track, i) => `${i + 1}. "${e(track.track)}" by ${e(track.artist)}`).join('\n')}</desc>
    <style>
        .header {
    ...
    ${recents
      ...
      <text y="${60 + i * 40}" class="name">${e(track.track)}</text>
