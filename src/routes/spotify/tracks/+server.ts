...
export async function GET({ url }) {
	try {
		...
		const tracks = await getTop('tracks', time, username, limit);

		for (let i = 0; i < limit; i++) {
			tracks[i].img = await dataUrl(tracks[i].img);
		}
		...
		const title = theme.custom_title ?? 'My top tracks of ' + titlePeriod;

		return text(
			`<svg width="350" height="${height}" viewBox="0 0 350 ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img">
    <title>${title}</title>
    <desc>${tracks.map((track, i) => `${i + 1}. "${e(track.track as string)}" by ${e(track.artist)}`).join('\n')}</desc>
    <style>
        .header {
        ...
        .name {
            ...
            font: 400 12.5px 'Segoe UI', Ubuntu, sans-serif;
    ...
    ${tracks
      ...
      <image href="${track.img}" y="${48.75 + i * 40}" clip-path="url(#clip${i})" class="image"></image>
      <text y="${60 + i * 40}" class="name">${e(track.track as string)}</text>
      <text y="${75 + i * 40}" class="artist">${e(track.artist)}</text>
      `
			)
			.join('\n')}
